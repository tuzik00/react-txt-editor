...
const StyleItems: StyleItemsType = {
    [BlockTypes.YOUTUBE]: {
        open: () => '::youtube[',
        close: (block) => {
            const id = block?.data?.id as string;

            if (!id) {
                return ']{}';
            }

            return `${block?.data?.alt as string || ''}]{#${id}}`;
        },
    },
    ...
    [BlockTypes.IMAGE]: {
        open: () => '![',
        close: (block) => `${block?.data?.alt as string || ''}](${block?.data?.src as string || ''} ${block?.data?.title as string || ''})`,
    },
    [BlockTypes.UL]: {
    ...
    [BlockTypes.HR]: {
        open: () => BlockStyleDict[BlockTypes.HR],
        close: () => BlockStyleDict[BlockTypes.UNSTYLED],
    },
    ...
    [InlineStyleTypes.SUPERSCRIPT]: {
        open: () => InlineStyleDict[InlineStyleTypes.SUPERSCRIPT],
        close: () => InlineStyleDict[InlineStyleTypes.SUPERSCRIPT],
    },
};
...
const draftToMd: RenderBlockType = (
    ...
    // Работа с деррективами
    if (customBlocks[type] && !StyleItems[type]) {
        const params = Object.entries(block.data || {})
            .map(([key, value]) => `${key}="${value as string}"`)
            .join(' ');

        markdownString += `::${type}[]{${params}}`;
